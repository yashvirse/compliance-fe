...
const AddActMasterPage: React.FC = () => {
  ...
  useEffect(() => {
    if (
      isEditMode &&
      currentActMaster &&
      Object.keys(departmentDropdown).length > 0
    ) {
      // Find department ID from department name
      const departmentId =
        Object.entries(departmentDropdown).find(
  ...
  return (
    <Box>
      ...
      <Box sx={{ mb: 3 }}>
        ...
        <Typography variant="body1" color="text.secondary">
          {isEditMode
            ? "Update act master information"
            : "Create a new act master entry"}
        </Typography>
      </Box>
                ...
                <Box sx={{ flex: "1 1 calc(50% - 12px)", minWidth: "250px" }}>
                  <CustomDropdown
                    ...
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        depaermentID: e.target.value as string,
                      })
                    }
                    options={Object.entries(departmentDropdown).map(
                      ([id, name]) => ({
                        label: name,
                        value: id,
                      })
      ...
      >
        ...
        >
          {success
            ? isEditMode
              ? "Act Master updated successfully!"
              : "Act Master added successfully!"
            : error}
        </Alert>
      </Snackbar>
    </Box>
