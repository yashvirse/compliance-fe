...
const SiteMap: React.FC<SiteMapProps> = ({
  ...
  const mapCenter =
    validSites.length > 0
      ? parseCoordinates(validSites[0].latitude, validSites[0].longtitude) ||
        defaultCenter
      : defaultCenter;
      ...
      {validSites.length === 0 && (
        ...
        >
          <Typography color="text.secondary">
            No sites with coordinates available
          </Typography>
        </Box>
      )}
      {validSites.length > 0 && (
        ...
        >
          <TileLayer
            {...({
              url: "https://mt1.google.com/vt/lyrs=m&x={x}&y={y}&z={z}&key=YOUR_GOOGLE_MAPS_API_KEY",
